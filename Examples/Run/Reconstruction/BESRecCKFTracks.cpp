...
void addRecCKFOptions(ActsExamples::Options::Description& desc) {
 ...
 opt("ckf-reco-mdc", bool_switch(),
      "Use reco mdc hits");
 opt("run-bes-upgrade", bool_switch(),
      "The detector geometry has pixel detector.");
}
 ...
 */
  ...
  RootBESMeasurementReader::Config BESMeasurementReaderCfg;
  BESMeasurementReaderCfg.filePath = inputDir + "/" + inputfiles.front();
  BESMeasurementReaderCfg.outputSimHits = "hits";
  BESMeasurementReaderCfg.outputParticles = "particles";
  BESMeasurementReaderCfg.trackingGeometry = trackingGeometry;
  BESMeasurementReaderCfg.randomNumbers = rnd;
  //BESMeasurementReaderCfg.readRecMdcHit = false;
  BESMeasurementReaderCfg.readRecMdcHit = vm["ckf-reco-mdc"].template as<bool>(); 
  BESMeasurementReaderCfg.runBesUpgrade = vm["run-bes-upgrade"].template as<bool>(); 
  if( BESMeasurementReaderCfg.runBesUpgrade){
    std::cout<<"Run with BESIII upgrade detector " << std::endl;
  }  
  sequencer.addReader(std::make_shared<RootBESMeasurementReader>(
      BESMeasurementReaderCfg, logLevel));
  ...
  particleSelectorCfg.inputParticles = BESMeasurementReaderCfg.outputParticles;
  particleSelectorCfg.inputMeasurementParticlesMap =
      BESMeasurementReaderCfg.outputMeasurementParticlesMap;
  ...
  if (truthSmearedSeeded) {
    ...
    spCfg.inputSourceLinks = BESMeasurementReaderCfg.outputSourceLinks;
    spCfg.inputMeasurements = BESMeasurementReaderCfg.outputMeasurements;
    ...
    if (truthEstimatedSeeded) {
      ...
      BESMeasurementReaderCfg.outputMeasurementSimHitsMap;
      trackFinderCfg.inputSimulatedHits = BESMeasurementReaderCfg.outputSimHits;
    ...
    paramsEstimationCfg.inputSourceLinks =
        BESMeasurementReaderCfg.outputSourceLinks;
  ...
  trackFindingCfg.inputMeasurements =
      BESMeasurementReaderCfg.outputMeasurements;
  trackFindingCfg.inputSourceLinks = BESMeasurementReaderCfg.outputSourceLinks;
...
//  for(const auto& measurement : BESMeasurementReaderCfg.outputMeasurements){
//    const auto sl = measurement.sourceLink();  
//    const auto sf = sl.geometryId();
//    externalSurfaces.push_back(sf); 
//  }
...
//  trackStatesWriter.inputParticles = BESMeasurementReaderCfg.outputParticles;
//  trackStatesWriter.inputSimHits = BESMeasurementReaderCfg.outputSimHits;
...
//      trackStatesWriter, logLevel));
  ...
  // filtered particle collection. This could be avoided when a seperate track
  // selection algorithm is used.
  trackSummaryWriter.inputParticles = BESMeasurementReaderCfg.outputParticles;
