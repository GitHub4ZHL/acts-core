  ...
  if (m_treeReader != nullptr && context.eventNumber < m_events) {
    ...
    if (m_treeReader->Next()) {
      ...
      for (size_t i = 0; i < ITKpositionX->GetSize(); ++i) {
        ...
        auto intersection =
            surfacePtr->intersect(context.geoContext, pos, mom.normalized(),
                                  Acts::BoundaryCheck(true));
        Acts::Vector3 posUpdated = intersection[0].position();
        ...
        for (auto h = moduleSimHits.begin(); h != moduleSimHits.end(); ++h) {
          ...
          if (surfacePtr->type() == Acts::Surface::SurfaceType::Cylinder) {
            ...
            Acts::ActsSquareMatrix<2> cov =
                Acts::ActsSquareMatrix<2>::Identity();
            cov(0, 0) = 0.1 * 0.1;
            cov(1, 1) = 0.4 * 0.4;
            ...
            Acts::ActsSquareMatrix<1> cov =
                Acts::ActsSquareMatrix<1>::Identity();
            cov(0, 0) = sigma * sigma;
      ...
      for (size_t i = 0; i < measurements.size(); i++) {
        ...
        auto geoId_ = sourceLink_->geometryId();
      }
    ...
    m_outputMeasurementParticlesMap(context,
                                    std::move(measurementParticlesMap));
    m_outputMeasurementSimHitsMap(context, std::move(measurementSimHitsMap));
    m_outputParticles(context, std::move(particles));
    m_outputSimHits(context, std::move(simHits));
