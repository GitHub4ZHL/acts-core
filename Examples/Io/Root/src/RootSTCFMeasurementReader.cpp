...
ActsExamples::RootSTCFMeasurementReader::RootSTCFMeasurementReader(
  ...
  ITDparticleId =
      new TTreeReaderArray<int>(*m_treeReader, "ITDHitCol.mcParticleIndex");
  ITDpositionX = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.position.x");
  ITDpositionY = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.position.y");
  ITDpositionZ = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.position.z");
  ITDmomentumX = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.momentum.x");
  ITDmomentumY = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.momentum.y");
  ITDmomentumZ = new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader,
                                                        "ITDHitCol.momentum.z");
  ITDtime =
      new TTreeReaderArray<Acts::ActsScalar>(*m_treeReader, "ITDHitCol.time");
  ...
  if (m_treeReader != nullptr && context.eventNumber < m_events) {
    ...
    if (m_treeReader->Next()) {
      ...
      for (size_t i = 0; i < ITDpositionX->GetSize(); ++i) {
        ...
        Acts::Vector3 pos((*ITDpositionX)[i], (*ITDpositionY)[i],
                          (*ITDpositionZ)[i]);
        Acts::Vector3 mom((*ITDmomentumX)[i], (*ITDmomentumY)[i],
                          (*ITDmomentumZ)[i]);

        // std::cout<<"r = " << std::hypot((*ITDpositionX)[i],
        // (*ITDpositionY)[i]) <<", phi = " << std::atan2((*ITDpositionY)[i],
        // (*ITDpositionX)[i]) << std::endl;
        ...
        if (cylinderSurface == nullptr) {
          std::cout << "Cast ITD surface to cylinder surface failed "
                    << std::endl;
        } else {
        ...
        auto energy =
            std::sqrt(mom.x() * mom.x() + mom.y() * mom.y() +
                      mom.z() * mom.z() + (*ITDmass)[i] * (*ITDmass)[i]);
      ...
      for (size_t i = 0; i < MDCcellID->GetSize(); ++i) {
        ...
        int cellID = (*MDCcellID)[i] == 0 ? m_MDCsCells[(*MDClayerID)[i]]
                                          : (*MDCcellID)[i];
        Acts::GeometryIdentifier geoId = Acts::GeometryIdentifier()
      ...
      for (auto simHitsGroup : groupByModule(simHits)) {
        ...
        for (auto h = moduleSimHits.begin(); h != moduleSimHits.end(); ++h) {
          ...
          if (surfacePtr->type() == Acts::Surface::SurfaceType::Cylinder) {
            ...
            Acts::ActsVector<2> par{m_ITDRadius[moduleGeoId.layer() / 2 - 1] *
      ...
      for (const auto& [particleIndex, nHits] : particleHitIdx) {
        std::cout << "particle " << particleIndex << " has " << nHits
                  << " nHits"
                  << ", nITDHits = " << particleITDHitIdx[particleIndex]
                  << std::endl;
      }

      std::cout << "nITDHits = " << nITDHits << ", nMDCHits = " << nMDCHits
                << ", nParticles (nITDHits>=3) = " << nParticles << std::endl;
    }
