...
std::vector<std::vector<FW::PixelSourceLink>> TelescopeTrackReader::operator()(
  ...
  while (ev_it != ev_it_end && good_datapack_count < nTracks) {
    ...
    for (size_t i = 0; i < 6; i++) {
      double x =
          frames[i]["hit"][0]["pos"][0].GetDouble() - pitchX * nPixX / 2.0;
      double y =
          frames[i]["hit"][0]["pos"][1].GetDouble() - pitchY * nPixY / 2.0;
      Acts::Vector2D loc;
      loc << x, y;
