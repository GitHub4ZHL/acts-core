   ...
   public:
    ...
    Result<void> filter(const Surface* surface, propagator_state_t& state,
      ...
      if (sourcelink_it != inputMeasurements.end()) {
        ...
        std::vector<FittableMeasurement<source_link_t>> fittableMeasurements;
        std::transform(
            sourcelinks.begin(), sourcelinks.end(),
            std::back_inserter(fittableMeasurements),
            [&](const auto& sl) { return m_calibrator(sl, boundParams); });
        ...
        auto sourcelinkSelectionRes = m_sourcelinkSelector(
            boundParams, fittableMeasurements, result.measurementChi2,
        ...
        for (const auto& index : result.measurementCandidateIndices) {
          ...
          auto addStateRes =
              addSourcelinkState(stateMask, boundState, sourcelinks.at(index),
                                 fittableMeasurements.at(index), isOutlier,
                                 result, state.geoContext, prevTip,
    ...
    Result<std::pair<size_t, TipState>> addSourcelinkState(
        ...
        const FittableMeasurement<source_link_t>& measurement, bool isOutlier,
        result_type& result,
        std::reference_wrapper<const GeometryContext> geoContext,
