...
{
   Double_t x =xx[0];
   Double_t f = p[0]*std::exp(-0.5*std::pow((x-p[1])/p[2],2)) + p[3] + p[4]*x + p[5]*std::pow(x,2);
   //Double_t f = p[0]*std::exp(-0.5*std::pow((x-p[1])/p[2],2));
   return f;
}


void anaHisto(TH1F* inputHist, int j, TH1F* meanHist, TH1F* widthHist, TCanvas* c, 
  ...
  if (inputHist->GetEntries() > 0) {
    if (fit) {
      ...
      if(gausFit) {
        ...
        fb->SetParameter(0,rt->Parameter(0)); 
        fb->SetParameter(1,rt->Parameter(1)); 
        fb->SetParameter(2,rt->Parameter(2)); 
      } 
      
      TFitResultPtr r = inputHist->Fit("fb", "S0");
      //c->cd(); 
      //inputHist->Draw(); 
      //fb->Draw("same"); 
      //Fit again 
...
//ana_momentum_angle_scan, try to fit with signal + bkg

void comparePerigee_particles_v3(
  ...
  // book hists
  auto fillHists = [&](std::vector<TrackSummaryReader>& readers,  std::map<std::string, std::vector<TH1F*>>& means_, std::map<std::string, std::vector<TH1F*>>& widths_, std::string tag, std::vector<TCanvas*>& cs){
    for (unsigned int ifile = 0; ifile < readers.size();++ifile) {
     ...
     if(plotResidual){
        std::pair<double, double> yRange_ = {-0.1, 0.1};
//        if( ps[ips]<0.125){
//           if(tag=="baseline"){ 
//	     yRange_ =  {-0.002,  0.004};
//	   } else {
...
//	}	
	...
	preSelectedRecoTracks = aTrackReader.getPreSelectedTracks(ievent, nMeasurementsMin, nOutliersMax, nHolesMax, absCosTheta);
	 ...
	 //cs.push_back(new TCanvas(Form("c_%s", hists[ivar]->GetName()), "", 500, 400));  
	 double scale = 1;
	 ...
	 //baseline_smearloc0_200_800_3_loc1_10000_800_3 
	   if(particle == "mu-" and tag == "baseline" and name.find("l1_")!=std::string::npos){
               ...
               if(ps[ips]==1.4 and deg=="0.5") scale = 0.94;
               if(ps[ips]==1.6 and deg=="0.5") scale = 0.97;
	   } 
	   if(particle == "pi-" and tag == "baseline" and name.find("pt_")!=std::string::npos){
             if(ps[ips]==0.2 and deg=="0") scale = scale*0.9;
	   ...
	   if(particle == "mu-" and tag == "baseline" and name.find("pt_")!=std::string::npos){
             if(ps[ips]==0.35 and deg=="0.5") scale = scale*0.98;
	   }
	 }
	

	// if(particle == "pi-" and deg=="0" and ps[ips]==0.1){
        //   if( name.find("l0_")!=std::string::npos ) scale = 1.2; 
        //   if( name.find("l1_")!=std::string::npos ) scale = 1.1; 
	// } 

	 anaHisto(hists[ivar], ips+1, means_[deg][ivar], widths_[deg][ivar], cs[ivar], fit, scale, false);
  ...
  std::vector<TCanvas*> cs_baseline;
  std::vector<TCanvas*> cs_upgrade;
  fillHists(tReaders0, means0, widths0, "baseline", cs_baseline);
  fillHists(tReaders1, means1, widths1, "upgrade", cs_upgrade);

  //TFile* fitResult = TFile::Open("fitResults.root","RECREATE");
  //for(auto c: cs_baseline)
  ...
  for(const auto& [deg, hists] : widths0){
    ...
    hists[1]->GetYaxis()->SetRangeUser(0., 5); 
  ...
  for(const auto& [deg, hists] : widths1){
    ...
    hists[1]->GetXaxis()->SetTitle(xAxisTitle.c_str());
    hists[1]->GetYaxis()->SetTitle("#sigma(z_{0}) [mm]");
    hists[1]->GetYaxis()->SetRangeUser(0.0, 5);
