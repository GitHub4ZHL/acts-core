...
void setHistStyle(hist_t* hist, short color = 1, short maker = 20) {
...
void setThisHistStyle(T* hist, short color = 1, short marker = 20,
  ...
  hist->GetYaxis()->SetNdivisions(nDivY);
...
void getRatio(TEfficiency * f1, TEfficiency * f2, TH1F* hist){
    TH1F* fTotal1 = (TH1F*)f1->GetTotalHistogram();
    TH1F* fTotal2 = (TH1F*)f2->GetTotalHistogram();

    int nBins1 = fTotal1->GetNbinsX();
    int nBins2 = fTotal2->GetNbinsX();
    if(nBins1 != nBins2){
      throw std::invalid_argument("The Nbins should be the same"); 
    }

    for(int i=1; i<nBins1+1; ++i){
      double eff1 = f1->GetEfficiency(i);
      double eff1Error = (f1->GetEfficiencyErrorUp(i) + f1->GetEfficiencyErrorLow(i))/2;
      double eff2 = f2->GetEfficiency(i);
      double eff2Error = (f2->GetEfficiencyErrorUp(i) + f2->GetEfficiencyErrorLow(i))/2;
      double ratio = eff2/eff1;
      double ratioError = ratio*std::hypot(eff1Error/eff1, eff2Error/eff2); 
      //std::cout<<"ratio = " << ratio << std::endl; 
      hist->SetBinContent(i, ratio); 
      hist->SetBinError(i, ratioError); 
    }
}
...
void replaceEff(TEfficiency* eff, std::string inputFile){
  TH1F* fTotal = (TH1F*)eff->GetTotalHistogram();
  int nBins = fTotal->GetNbinsX();

  // Read in file and fill values
  std::ifstream inputStream(inputFile.c_str(), std::ios::in);
  if (not inputStream.good()) {
    throw std::invalid_argument("The " + inputFile + " does not exsit!");
  }
  ...
  while (std::getline(inputStream, line)) {
    ...
    std::istringstream tmp(newLine);

    double binNumber, binValue, binContent, passedEvents, totalEvents;
    std::string del; 
    tmp >> binNumber >>  binValue >>  totalEvents >>  passedEvents;

    std::cout<<binValue<<" " << binContent <<" " << totalEvents <<" " << passedEvents << std::endl;


    eff->SetPassedEvents (iline+1, passedEvents);
    eff->SetTotalEvents (iline+1, totalEvents);
...
template <typename eff_t>
void setEffRange(eff_t* eff, float minX, float maxX, float minY, float maxY,
	         float xTitleSize = 0.05, float yTitleSize = 0.05,
                 float xLabelSize = 0.05, float yLabelSize = 0.05,
                 float xTitleOffset = 1.2, float yTitleOffset = 1.2
		) {
