...
void comparePerigee_particles_pixelR(
  ...
  std::string saveTag ="PixelR_" + particle + "_res"; 
  ...
  tReaders0.reserve(pixelRs.size()*ps.size());
  tReaders1.reserve(pixelRs.size()*ps.size());
  ...
  for(int i=0; i<pixelRs.size(); ++i){
	...
	std::string psstring = std::to_string(static_cast<int>(ps[j]*1000));
	  ...
	  inputTrackSummaryFileNames1.push_back(TFile::Open(inFile.c_str(), "read"));
        ...
        if(k==0){	
          chains0[pixelRs[i]].push_back(treeChain); 
        } else {
          chains1[pixelRs[i]].push_back(treeChain); 
	}
      }
    }
  }
  ...
  for(int i=0; i<pixelRs.size(); ++i){
    for(int j=0; j<names.size(); ++j){
      means0[pixelRs[i]].push_back(new TH1F(Form("%s_%s_mean0", names[j].c_str(), pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() )); 
      widths0[pixelRs[i]].push_back(new TH1F(Form("%s_%s_width0", names[j].c_str(), pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() )); 
    }
    means0[pixelRs[i]].push_back(new TH1F(Form("pt_%s_mean0", pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() )); 
    widths0[pixelRs[i]].push_back(new TH1F(Form("pt_%s_width0", pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() )); 
  }

  for(int i=0; i<pixelRs.size(); ++i){
    for(int j=0; j<names.size(); ++j){
      means1[pixelRs[i]].push_back(new TH1F(Form("%s_%s_mean1", names[j].c_str(), pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() ));
      widths1[pixelRs[i]].push_back(new TH1F(Form("%s_%s_width1", names[j].c_str(), pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() ));
    }
    means1[pixelRs[i]].push_back(new TH1F(Form("pt_%s_mean1", pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() ));
    widths1[pixelRs[i]].push_back(new TH1F(Form("pt_%s_width1", pixelRs[i].c_str()), "", ptbins.size()-1, ptbins.data() ));
  }
  ...
  auto fillHists = [&](std::vector<TrackSummaryReader>& readers,  std::map<std::string, std::vector<TH1F*>>& means_, std::map<std::string, std::vector<TH1F*>>& widths_, std::string tag = "baseline"){
    for (unsigned int ifile = 0; ifile < readers.size();++ifile) {
     int ir = ifile/ps.size();
     auto pixelR  = pixelRs[ir];
     int ips= ifile - ir*ps.size();
     ...
     for (int ivar = 0;ivar < names.size(); ++ivar) {
	  ...
	  if(ps[ips] > 1.2){
       ...
       hists.push_back(new TH1F(Form("%s_pixelR%i_p%i_%s", name.c_str(),  ir,  ips, tag.c_str()),  "",  200, yRange.first, yRange.second));
     }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for(const auto& [deg, hists] : widths0){
    ...
    legs[6]->AddEntry(hists[6], Form("%s",tags[deg].c_str()), "APL");
    i++;
  }
