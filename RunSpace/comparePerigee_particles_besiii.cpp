...
void comparePerigee_particles_besiii(
       ...
       std::vector<std::string> absCosThetas = {"0.8"},
  ...
  for(int i=0; i<absCosThetas.size(); ++i){
    for(int k=0; k<inputPaths.size(); ++k){
      ...
      for(int j=0; j<ps.size(); ++j){
        ...
        std::string inFile = inputPath + tag+ "/" + momentumType + "/" + particle + "/CosThetaDeg_-" + absCosThetas[i] + "_" + absCosThetas[i] + "_momentumMev_" + psstring + "_" + psstring + "/tracksummary_ckf.root";
  ...
  fillHists(tReaders0, means0, widths0, "upgrade");


  TH1F* hist_baseline_d0_mu = new TH1F("baseline_d0_mu", "",  ptbins.size()-1, ptbins.data());
  TH1F* hist_baseline_z0_mu = new TH1F("baseline_z0_mu", "",  ptbins.size()-1, ptbins.data());
  TH1F* hist_baseline_pt_mu = new TH1F("baseline_pt_mu", "",  ptbins.size()-1, ptbins.data());
  TH1F* hist_baseline_d0_pi = new TH1F("baseline_d0_pi", "",  ptbins.size()-1, ptbins.data());
  TH1F* hist_baseline_z0_pi = new TH1F("baseline_z0_pi", "",  ptbins.size()-1, ptbins.data());
  TH1F* hist_baseline_pt_pi = new TH1F("baseline_pt_pi", "",  ptbins.size()-1, ptbins.data());



  for(int i=0; i<hist_baseline_d0_mu->GetNbinsX(); ++i){
    hist_baseline_d0_mu->SetBinContent(i+1, res_baseline_d0_mu[i]);
    hist_baseline_z0_mu->SetBinContent(i+1, res_baseline_z0_mu[i]);
    hist_baseline_pt_mu->SetBinContent(i+1, res_baseline_pt_mu[i]);
    hist_baseline_d0_mu->SetBinError(i+1, res_baseline_d0_mu[i]*0.0001);
    hist_baseline_z0_mu->SetBinError(i+1, res_baseline_z0_mu[i]*0.0001);
    hist_baseline_pt_mu->SetBinError(i+1, res_baseline_pt_mu[i]*0.0001);

    hist_baseline_d0_pi->SetBinContent(i+1, res_baseline_d0_pi[i]);
    hist_baseline_z0_pi->SetBinContent(i+1, res_baseline_z0_pi[i]);
    hist_baseline_pt_pi->SetBinContent(i+1, res_baseline_pt_pi[i]);
    hist_baseline_d0_pi->SetBinError(i+1, res_baseline_d0_pi[i]*0.0001);
    hist_baseline_z0_pi->SetBinError(i+1, res_baseline_z0_pi[i]*0.0001);
    hist_baseline_pt_pi->SetBinError(i+1, res_baseline_pt_pi[i]*0.0001);
  }

  TH1F* hist_baseline_d0 = nullptr;
  TH1F* hist_baseline_z0 = nullptr;
  TH1F* hist_baseline_pt = nullptr;
  if(particle=="mu-"){
    hist_baseline_d0 = hist_baseline_d0_mu; 
    hist_baseline_z0 = hist_baseline_z0_mu; 
    hist_baseline_pt = hist_baseline_pt_mu; 
  } else if(particle=="pi-"){
    hist_baseline_d0 = hist_baseline_d0_pi; 
    hist_baseline_z0 = hist_baseline_z0_pi; 
    hist_baseline_pt = hist_baseline_pt_pi; 
  }
  ...
  //c1->SetGrid();  
  int i=0; 
  setThisHistStyle(hist_baseline_d0, colors0[i], markers0[i], xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
  hist_baseline_d0->Draw("EsameX0");
  hist_baseline_d0->GetXaxis()->SetTitle(xAxisTitle.c_str());
  hist_baseline_d0->GetYaxis()->SetTitle("#sigma(d_{0}) [mm]");
  hist_baseline_d0->GetYaxis()->SetRangeUser(0, 1);
  legs[0]->AddEntry(hist_baseline_d0, "MDC", "APL");
  ...
  for(const auto& [deg, hists] : widths0){
    ...
    legs[0]->AddEntry(hists[0], "MDC + Pixel", "APL");


    for(int i=0; i<hists[0]->GetNbinsX(); ++i){
       double baseline = hist_baseline_d0->GetBinContent(i+1); 
       double upgrade = hists[0]->GetBinContent(i+1); 
       std::cout<<"d0 diff at "<< ps[i] << " is " << (baseline-upgrade)/baseline * 100  <<" % " << std::endl; 
    }
  ...
  i=0; 
  hist_baseline_z0->Draw("EsameX0");
  setThisHistStyle(hist_baseline_z0, colors0[i], markers0[i], xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
  hist_baseline_z0->GetXaxis()->SetTitle(xAxisTitle.c_str());
  hist_baseline_z0->GetYaxis()->SetTitle("#sigma(z_{0}) [mm]");
  hist_baseline_z0->GetYaxis()->SetRangeUser(0, 3.2);
  legs[1]->AddEntry(hist_baseline_z0, "MDC", "APL");
  ...
  for(const auto& [deg, hists] : widths0){
    ...
    legs[1]->AddEntry(hists[1], "MDC + Pixel", "APL");
    for(int i=0; i<hists[1]->GetNbinsX(); ++i){
       double baseline = hist_baseline_z0->GetBinContent(i+1); 
       double upgrade = hists[1]->GetBinContent(i+1); 
       std::cout<<"z0 diff at "<< ps[i] << " is " << (baseline-upgrade)/baseline * 100  <<" % " << std::endl; 
    }
    i++;
  }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  i=0; 
  hist_baseline_pt->Draw("EsameX0");
  setThisHistStyle(hist_baseline_pt, colors0[i], markers0[i], xTitleSize, yTitleSize,
                         xLabelSize, yLabelSize, xTitleOffset, yTitleOffset);
  ...
  for(const auto& [deg, hists] : widths0){
    ...
    legs[6]->AddEntry(hists[6], "MDC + Pixel", "APL");
    for(int i=0; i<hists[6]->GetNbinsX(); ++i){
       double baseline = hist_baseline_pt->GetBinContent(i+1); 
       double upgrade = hists[6]->GetBinContent(i+1); 
       std::cout<<"pt diff at "<< ps[i] << " is " << (baseline-upgrade)/baseline * 100  <<" % " << std::endl; 
    }
    i++;
  }
